def yolov7_example(img='/home/shreejan/Documents/yolov7/woman.jpg', webcam=False):
    """
    Args:
        img: To detect the objects in an image, webcam must not be set to true. If link to an image is not provided,
    ...
    """
    line_thickness = 1
    yolov7_instance = get_yolov7_ob()

    if webcam:
        """
        Original detect.py 's LoadWebcam
        """
        ...
        while True:
            ...
            align_bottom = img.shape[0] - 100
            align_right = (img.shape[1] / 1.45)
            if ret:
                coordinates, names, object_counts = yolov7_instance.detect(img, conf_thres=0.5)
                img = draw_bboxes(coordinates, names,  img, line_thickness)
                for key, value in object_counts.items():
                    kv = f"{key}={value}"
                    align_bottom = align_bottom - 35
                    cv2.putText(img, str(kv), (int(align_right), align_bottom), cv2.FONT_HERSHEY_SIMPLEX, 1, (45, 255, 26), 1)
        ...
        img = cv2.imread(img)
        coordinates, names, object_counts = yolov7_instance.detect(img)
        img = draw_bboxes(coordinates, names, img, line_thickness)
        cv2.namedWindow('img', cv2.WINDOW_NORMAL)
        cv2.imshow('img', img)
        cv2.waitKey(0)
...
if __name__ == '__main__':
    from utils.get_yolov7_utils import get_yolov7_ob
    import cv2
    import random

    yolov7_example(webcam=True)